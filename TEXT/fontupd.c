...
//fsize:整个文件大小
//pos:当前文件指针位置
static u32 _fupd_prog(u16 x, u16 y, u8 size, u32 fsize, u32 pos)
{
    float prog;
    u8 t = 0XFF;
...
//返回值:0,成功;其他,失败.
static u8 _updata_fontx(u16 x, u16 y, u8 size, u8 *fxpath, u8 fx)
{
    u32 flashaddr = 0;
    FIL * fftemp;
    u8 *tempbuf;
    ...
    if(rval == 0)
    {
        ...
        while(res == FR_OK) //死循环执行
        {
            ...
            offx += bread;
            _fupd_prog(x, y, size, fftemp->fsize, offx);         //进度显示
...
u8 update_font(u16 x, u16 y, u8 size, u8* src)
{
    ...
    if(rval == 0) //字库文件都存在.
    {
        ...
        {
            _fupd_prog(x + 20 * size / 2, y, size, FONTSECSIZE, i); //进度显示
        ...
        strcpy((char*)pname, (char*)src);               //copy src内容到pname
        strcat((char*)pname, (char*)UNIGBK_PATH);
        res = _updata_fontx(x + 20 * size / 2, y, size, pname, 0); //更新UNIGBK.BIN
        if(res)
        {
            myfree(SRAMIN, pname);
            return 1;
        ...
        strcpy((char*)pname, (char*)src);               //copy src内容到pname
        strcat((char*)pname, (char*)GBK12_PATH);
        res = _updata_fontx(x + 20 * size / 2, y, size, pname, 1); //更新GBK12.FON
        if(res)
        {
            myfree(SRAMIN, pname);
            return 2;
        ...
        strcpy((char*)pname, (char*)src);               //copy src内容到pname
        strcat((char*)pname, (char*)GBK16_PATH);
        res = _updata_fontx(x + 20 * size / 2, y, size, pname, 2); //更新GBK16.FON
        if(res)
        {
            myfree(SRAMIN, pname);
            return 3;
        ...
        strcpy((char*)pname, (char*)src);               //copy src内容到pname
        strcat((char*)pname, (char*)GBK24_PATH);
        res = _updata_fontx(x + 20 * size / 2, y, size, pname, 3); //更新GBK24.FON
        if(res)
        {
            myfree(SRAMIN, pname);
            return 4;
